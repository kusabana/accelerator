    ...
    if isinstance(hlil_instruction, binaryninja.HighLevelILInstruction):
        ...
        for operand in hlil_instruction.operands:
            if isinstance(operand, list):
                operands.append('(' + ', '.join(hlil_to_expression(inner) for inner in operand) + ')')
            else:
...
expressions = {
    'CL_GetDownloadQueueSize': ('HLIL_IF(HLIL_CMP_E(HLIL_CALL(HLIL_CONST_PTR(??), ()), HLIL_CONST(0)))', ), # universal
    'CL_DownloadUpdate': (
...
def spawn(binary):
    ...
    with binaryninja.open_view(binary) as bv:
        ...
        values = [(name, expression_extract_wildcard(hlil, expr)) for name, expr in expressions.items()]
        values = [(name, (pattern, int(val))) for name, (pattern, val) in values if val is not None]
...
if __name__ == '__main__':
    print('~ NOTE: this script takes a while to run due to binaryninja\'s analysis process.')
